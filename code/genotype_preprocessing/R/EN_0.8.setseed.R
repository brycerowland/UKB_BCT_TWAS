...
#Check which variants are in reflist
training_snps <- paste(snp.list[,1], snp.list[,2], snp.list[,3], snp.list[,4], sep = ":")
nonflip_index <- which(training_snps %in% ref_var_list[,1])
length(training_snps)


training_snps_flipped <- paste(snp.list[,1], snp.list[,2], snp.list[,4], snp.list[,3], sep = ":")


#Are any snps in the reference list (GWAS dataset), but have flipped references also well imputed in eQTL data? If so, drop their flipped references.
drop_index <- which((training_snps %in% training_snps_flipped)& (training_snps_flipped %in% ref_var_list[,1])) 
n_snps_dropped <- length(drop_index)

if (any((training_snps %in% training_snps_flipped)& (training_snps_flipped %in% ref_var_list[,1]))){
	training_snps <- training_snps[-drop_index]
	training_snps_flipped <- training_snps_flipped[-drop_index]
	snp.list <- snp.list[-drop_index,]
	X.dosage <- X.dosage[,-drop_index]
...
cat(paste0("There are ", n.snps.dosage, " SNPs in the dosage file matching to the ref file\n"))
cat(paste0("There are ", n.snp.list, " SNPs in the SNP list matching to the ref file\n"))
...
if (n.snps.window < 2) {
	...
	cat(paste0("There are ", n_snps_flipped, " SNPs flipped to align with ref file\n"), file = paste0(opt$o, ".log"), append = T)
	cat(paste0("There are ", n_snps_dropped, " SNPs dropped to align with ref file\n"))
        cat(paste0("There are ", n_snps_dropped, " SNPs dropped to align with ref file\n"), file = paste0(opt$o, ".log"), append = T)
